...
- **Atomic, multi-document approval.** Provisioning a recruiter account, creating a company, and updating the application must all succeed or all fail. This was implemented with a MongoDB transaction — which in turn requires running MongoDB as a replica set and threading the session through every write. Side effects like sending the credentials email were deliberately moved **after** the commit (a transaction can retry its callback).
- **Stateful sessions on top of stateless JWTs.** Plain JWTs can't be revoked. Combining a per-token `jti` blocklist and a `credentialsChangedAt` timestamp in Redis gives instant "log out this device" and "log out everywhere" without a per-request DB hit for the common case.
- **OTP abuse resistance.** Verification codes live entirely in Redis with hashed values, attempt counters, and escalating block windows — so no half-finished state is written to the database and brute-forcing is throttled.
- **Type-safe partial updates.** Supporting PATCH semantics for nested objects (job `location`/`salary`) meant validating partial nested Zod schemas and writing field-by-field with dot-notation so unsent fields are preserved — while keeping cross-field invariants (e.g. *city + country required unless remote*) correct against the merged result.
- **Status as a state machine, not a field.** Job status changes are exposed only through dedicated `publish`/`close` endpoints (with allowed source states) instead of a free-form `status` field on the generic update, preventing illegal transitions.
- **Typed repository abstraction.** Designing `DatabaseRepo<T>` with TypeScript overloads (e.g. `lean: true/false` changing the return type) keeps query call-sites both DRY and precisely typed.
...
2. Keep the layered architecture and conventions described in `CLAUDE.md` (import models from `models/index.ts`, use enums instead of string literals, throw `AppError` subclasses, pass errors via `next()`).
3. Ensure it type-checks: `npx tsc --noEmit` from `server/`.
